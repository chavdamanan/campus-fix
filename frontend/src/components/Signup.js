...
export default function Signup({ onAuth }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (ok) {
        const user = { name, email, role, mobile, department };
        setCurrentUser(user);
        if (onAuth) onAuth();
        const target = role === 'worker' ? '/worker' : '/dashboard';
        navigate(target);
      } else {
      ...
      <div className="card">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginTop: 14, textAlign: 'center' }}>
            <button type="submit" className="btn large" disabled={loading}>{loading ? 'Creating...' : 'Create Account'}</button>
          </div>
        </form>
